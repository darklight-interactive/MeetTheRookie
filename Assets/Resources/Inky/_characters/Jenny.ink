...
{IsQuestComplete(Level4_3_Quests.hosi_highscore):
     Well, <i>I'm</i> not a cop.
     And I'm not the one blocking you from your high score.
    ~ SetSpeaker(Speaker.Jenny)
     ...
...
{IsQuestActive(jenny_KR):
    ...
    ~ SetSpeaker(Speaker.Jenny)
     Case in point.
    ~ SetSpeaker(Speaker.Lupe)
    ~ CompleteQuest(jenny_KR)
        -> DONE
}

{IsQuestActive(jenny_personal_info):
    ~ SetSpeaker(Speaker.Jenny)
     Mhm.
    ~ SetSpeaker(Speaker.Lupe)
     What do you do for fun?
...
* The Old Winery on the hill... 
    ~ StartQuest(jenny_winery)
    -> DONE

* You're a local? 
    ~ StartQuest(jenny_personal_info)
    -> DONE
